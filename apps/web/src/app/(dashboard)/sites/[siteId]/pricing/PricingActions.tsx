...
export default function PricingActions({ type, siteId, priceBookId, rateRuleId, bookStatus, unitTypes }: Props) {
  ...
  async function doAction(url: string, method: string, body?: object) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message ?? t('dashboard.sites.pricing.errorGeneric'));
      router.refresh();
      setShowForm(false);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : t('dashboard.sites.pricing.errorGeneric'));
    } finally {
      setLoading(false);
    }
  }
  ...
  if (type === 'create-book') {
    return showForm ? (
      ...
      >
        <input name="name" placeholder={t('dashboard.sites.pricing.bookNamePlaceholder')} required style={{ ...inputStyle, width: '160px' }} />
        ...
        <button type="submit" disabled={loading} style={{ background: loading ? '#e2e8f0' : '#0f172a', color: loading ? '#94a3b8' : '#fff', border: 'none', borderRadius: '8px', padding: '9px 16px', fontWeight: 700, fontSize: '13px', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          {loading ? '…' : t('dashboard.sites.pricing.create')}
        </button>
        <button type="button" onClick={() => setShowForm(false)} style={{ ...btnStyle, background: 'none', border: 'none' }}>{t('dashboard.sites.pricing.cancel')}</button>
      ...
      >
        {t('dashboard.sites.pricing.newPriceBook')}
      </button>
    );
  }
    ...
    return (
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
        {bookStatus === 'draft' && (
          ...
          >
            {loading ? '…' : t('dashboard.sites.pricing.publish')}
          </button>
        )}
        {bookStatus === 'published' && (
          ...
          >
            {loading ? '…' : t('dashboard.sites.pricing.archive')}
          </button>
        )}
  ...
  if (type === 'remove-rule') {
    return (
      ...
      >
        {loading ? '…' : t('dashboard.sites.pricing.remove')}
      </button>
    );
  }
  ...
  return showForm ? (
    ...
    >
      <select name="unitTypeId" required style={inputStyle}>
        <option value="">{t('dashboard.sites.pricing.unitTypeSelectPlaceholder')}</option>
        {unitTypes.map((ut) => <option key={ut.id} value={ut.id}>{ut.name}</option>)}
      </select>
      <input name="amountEur" type="number" step="0.01" min="0" placeholder={t('dashboard.sites.pricing.pricePlaceholder')} required style={{ ...inputStyle, width: '110px' }} />
      ...
      <button type="submit" disabled={loading} style={{ background: loading ? '#e2e8f0' : '#0f172a', color: loading ? '#94a3b8' : '#fff', border: 'none', borderRadius: '8px', padding: '9px 16px', fontWeight: 700, fontSize: '13px', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        {loading ? '…' : t('dashboard.sites.pricing.addRule')}
      </button>
      <button type="button" onClick={() => setShowForm(false)} style={{ ...btnStyle, background: 'none', border: 'none' }}>{t('dashboard.sites.pricing.cancel')}</button>
      {error && <p style={{ fontSize: '12px', color: '#dc2626', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif", width: '100%' }}>{error}</p>}
    </form>
    ...
    <button onClick={() => setShowForm(true)} style={{ background: 'none', border: 'none', fontSize: '13px', fontWeight: 600, color: '#64748b', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      {t('dashboard.sites.pricing.addRateRule')}
    </button>
  );
}
