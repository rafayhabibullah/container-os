    ...
    <>
      ...
      >
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#94a3b8', marginBottom: '6px' }}>{t('dashboard.reservations.drawer.title')}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <span style={{
                ...
                {t(`dashboard.reservations.status.${reservation.status}`)}
              </span>
              <span style={{ fontSize: '12px', color: '#94a3b8' }}>{t('dashboard.reservations.drawer.via', { source: reservation.source })}</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '14px 16px' }}>
            <SectionHeader title={t('dashboard.reservations.drawer.customer')} icon={
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <Field label={t('dashboard.reservations.drawer.moveInDate')} value={fmt(reservation.startDate)} />
            <Field label={t('dashboard.reservations.drawer.expires')} value={fmt(reservation.expiresAt)} />
            <Field label={t('dashboard.reservations.drawer.created')} value={fmtTime(reservation.createdAt)} />
          </div>

          {/* Unit */}
          <div style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '14px 16px' }}>
            <SectionHeader title={t('dashboard.reservations.drawer.unit')} icon={
            ...
            {fetching ? (
              ...
              <span style={{ fontSize: '12px', color: '#f87171' }}>{t('dashboard.reservations.drawer.loadError')}</span>
            ) : unit ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <Field label={t('dashboard.reservations.drawer.unitCode')} value={<span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '14px' }}>{unit.unitCode}</span>} />
                <Field label={t('dashboard.reservations.drawer.kind')} value={unit.kind.replace(/_/g, ' ')} />
                <Field label={t('dashboard.reservations.drawer.status')} value={
                ...
                <Field label={t('dashboard.reservations.drawer.driveUp')} value={unit.driveUp ? t('dashboard.reservations.drawer.yes') : t('dashboard.reservations.drawer.no')} />
                {unit.conditionState && <Field label={t('dashboard.reservations.drawer.condition')} value={unit.conditionState} />}
              </div>
            ) : (
          ...
          {/* Unit Type */}
          <div style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '14px 16px' }}>
            <SectionHeader title={t('dashboard.reservations.drawer.unitType')} icon={
            ...
            {fetching ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <Field label={t('dashboard.reservations.drawer.name')} value={<span style={{ fontWeight: 700 }}>{unitType.name}</span>} />
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <Field label={t('dashboard.reservations.drawer.size')} value={`${unitType.sizeSqm} m²${unitType.sizeCbm ? ` / ${unitType.sizeCbm} m³` : ''}`} />
                  <Field label={t('dashboard.reservations.drawer.doorType')} value={unitType.doorType?.replace(/_/g, ' ') ?? null} />
                </div>
                {unitType.features.length > 0 && (
                  <div>
                    <span style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#94a3b8', display: 'block', marginBottom: '6px' }}>{t('dashboard.reservations.drawer.features')}</span>
          ...
          {/* Site */}
          <div style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '14px 16px' }}>
            <SectionHeader title={t('dashboard.reservations.drawer.site')} icon={
            ...
            {fetching ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <Field label={t('dashboard.reservations.drawer.name')} value={<span style={{ fontWeight: 700 }}>{site.name}</span>} />
                <Field label={t('dashboard.reservations.drawer.address')} value={`${site.address.street}, ${site.address.postalCode} ${site.address.city}, ${site.address.country}`} />
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <Field label={t('dashboard.reservations.drawer.status')} value={site.status} />
                  <Field label={t('dashboard.reservations.drawer.currency')} value={site.currency} />
                  <Field label={t('dashboard.reservations.drawer.timezone')} value={site.timezone} />
                </div>
              </div>
            ) : (
